...
static PlannedStmt *pg_carbon_planner(Query *parse, const char *query_string, int cursorOptions,
    ...
    if (pg_carbon_enable && parse->commandType == CMD_SELECT) {
        ...
        elog(WARNING, "pg carbon generate plan failed❌, fallback to standard planner!");
    }
...
void _PG_init(void) {
    ...
    DefineCustomBoolVariable("pg_carbon.enable", "Enable pg_carbon optimizer", NULL,
                             &pg_carbon_enable, true, PGC_USERSET, 0, NULL, NULL, NULL);
