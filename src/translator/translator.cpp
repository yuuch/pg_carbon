...
Expression *Translator::TranslateQueryToCarbon(Query *pg_query) {
    ...
    if (pg_query->rtable && pg_query->jointree && pg_query->jointree->fromlist) {
        ...
        if (list_length(fromlist) == 1) {
            ...
            if (IsA(node, RangeTblRef)) {
                ...
                RangeTblEntry *rte = (RangeTblEntry *)list_nth(pg_query->rtable, rtr->rtindex - 1);

                if (rte->rtekind == RTE_RELATION) {
                    Operator *scan_op = new LogicalGet(rte->relid, rtr->rtindex);
                    current_expr = new Expression(scan_op);
                }
            }
        }
    }
    ...
    if (pg_query->jointree->quals) {
        auto filter_op = new LogicalFilter(pg_query->jointree->quals);
        PgVector<Expression *> children;
        children.push_back(current_expr);
        current_expr = new Expression(filter_op, children);
    }
    ...
    if (pg_query->limitOffset || pg_query->limitCount) {
        auto limit_op = new LogicalLimit(pg_query->limitOffset, pg_query->limitCount);
        PgVector<Expression *> children;
        children.push_back(current_expr);
        current_expr = new Expression(limit_op, children);
    }
    ...
    if (pg_query->targetList) {
        ...
        PgVector<Expression *> children;
        children.push_back(current_expr);
        current_expr = new Expression(projection_op, children);
    }

    return current_expr;
}

Plan *Translator::TranslatePlanToPG(Memo *memo, GroupExpression *best_physical_plan,
                                    Query *pg_query) {
    ...
    for (int i = 0; i < 1000; ++i) { // Limit to avoid infinite loop if bug
        ...
        if (singleton.IsSubset(output_cols)) {
            ...
            if (col->GetType() == CarbonColumnType::TABLE_COLUMN) {
                ...
                Var *var = makeVar(tc->GetRtIndex(), tc->GetAttrNum(), INT4OID, -1, InvalidOid,
                                   0); // Mock types for now
                tle = makeTargetEntry((Expr *)var, (AttrNumber)list_length(target_list) + 1,
                                      pstrdup("col"), false);
                ...
                tle = makeTargetEntry((Expr *)copyObjectImpl(ec->GetExpr()),
                                      (AttrNumber)list_length(target_list) + 1, pstrdup("expr"),
                                      false);
            }
